...
using System.Linq;
using System.Text;
using UnitTests.Storage.BasicBase.Mock;
...
namespace UnitTests.Storage.BasicBase {
    [TestFixture]
    public class BasicBaseTest {
        private BasicMockManager CreateSampleDataSet() {
            BasicMockManager manager = new BasicMockManager();
            BasicDataSet set = BasicDataSet.Create();
            manager.Init(set);
        ...
        //    BasicMockManager manager = CreateSampleDataSet();
        //    string testName = "test folder";
        //    manager.GetRootFolder().CreateSubFolder(testName);

        //    IList<IFolder> folders = manager.GetRootFolder().GetSubFolders(DateTime.Now);
        //    Assert.IsTrue(folders.Count == 1);
        //    IFolderInstance folderInstance = folders[0].GetFolderInstance(DateTime.Now);
        //    Assert.IsNotNull(folderInstance);
        //    Assert.IsTrue(folderInstance.Name == testName);
        //}
